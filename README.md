...
- Full Excalidraw editor with self-hosted handwritten and multilingual fonts
- Multiple document tabs with explicit saved, saving, conflict, and recovery states
- Debounced autosave with disk-conflict detection and deletion recovery
...
- **State:** Zustand
- **Filesystem safety:** atomic writes, expected-content hashes, same-file identity checks, and serialized save transactions

Frontend code lives in `src/`; native commands, file watching, and persistence live in `src-tauri/`.
